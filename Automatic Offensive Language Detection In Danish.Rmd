...
library(pacman)
p_load(readr, tidyverse,rsample,recipes, textrecipes, parsnip, yardstick,workflows, discrim,kernlab,stringr, tm, ggplot2, GGally, e1071, caret,stopwords, stringi, SnowballC,fastmatch, parsnip, keras, tensorflow, ranger, MLmetrics)
...
Setting the model specifications (tidymodels framework) to make them ready for classification:
```{r}
...
### Random Forest:
```{r}
bind_cols(testing,predictions_RF) %>% conf_mat(label, .pred_class) 
```
...
## F1-Scores
```{r}
testing$label<-as.character(as.numeric(testing$label)-1)
predictions_RF$.pred_class<-as.character(as.numeric(predictions_RF$.pred_class)-1)
predictions_NB$.pred_class<-as.character(as.numeric(predictions_NB$.pred_class)-1)
predictions_SVM$.pred_class<-as.character(as.numeric(predictions_SVM$.pred_class)-1)
predictions_log$.pred_class<-as.character(as.numeric(predictions_log$.pred_class)-1)
...
### Random Forest
```{r}
F1_Score_macro(y_true= testing$label, y_pred = predictions_RF$.pred_class)
```

### Logistic Regression
```{r}
F1_Score_macro(y_true= testing$label, y_pred = predictions_log$.pred_class)
```

### Naive Bayes
```{r}
F1_Score_macro(y_true= testing$label, y_pred = predictions_NB$.pred_class)
```

### SVM
```{r}
F1_Score_macro(y_true= testing$label, y_pred = predictions_SVM$.pred_class)
```
...
```{r}
training$label<-as.numeric(training$label)
testing$label<-as.numeric(testing$label)
...
max_features <- 1000 #Should be the same as tokenizing
batch_size <- 32
...
predict_classes_lstm <- predict_classes(model_lstm, x_test)
bind_cols(testing, predictions_log) %>% conf_mat(label, .pred_class) 
...
## CNN:
# Change to: https://github.com/rstudio/keras/blob/master/vignettes/examples/imdb_cnn.R ?
It is currently CNN_LSTM (https://github.com/rstudio/keras/blob/master/vignettes/examples/imdb_cnn_lstm.R)
...
```{r}
#Initialize model
model_cnn_real <- keras_model_sequential()
...
history_cnn_real <- model_cnn_real %>%
  fit(
    x_train, y_train,
    batch_size = batch_size,
    epochs = 15,
...
```{r}
plot(history_cnn_real,method= "ggplot2", smooth = TRUE)

model_cnn_real %>% plot_model() 
```
